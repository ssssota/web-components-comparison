<svelte:options
  customElement={{
    tag: "svelte-feed-card",
    shadow: "open",
    props: {
      postId: { attribute: "post-id", type: "Number" },
    },
  }}
/>
...
<script lang="ts">
  ...
  let { postId = 1 }: { postId?: string | number } = $props();

  let status = $state<FeedStatus>("loading");
  let post = $state<Post | null>(null);
  let user = $state<User | null>(null);
  let reactions = $state<ReactionMap>(createReactions(1));
  let error = $state("");
  let saveError = $state("");
  ...
  const id = $derived(parsePostId(postId));
  ...
  async function load(currentId = id) {
    ...
    try {
      const data = await loadFeed(currentId, loadController.signal);
      post = data.post;
      user = data.user;
      reactions = createReactions(currentId);
      status = "ready";
    } catch (err) {
  ...
  async function react(reactionId: ReactionId) {
    ...
    try {
      await persistReactions(id, next, saveController.signal);
    } catch (err) {
      if (isAbortError(err)) return;
      reactions = previous;
      saveError = "保存に失敗しました";
...
<div class="card" data-status={status}>
  ...
  {:else if status === "error"}
    <div class="error">
      <p>{error}</p>
      <button type="button" class="retry" onclick={() => load()}>再試行</button>
    </div>
  {:else if status === "ready" && post && user}
    <header class="header">
